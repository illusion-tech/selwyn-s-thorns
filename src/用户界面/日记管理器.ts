    ...
    async 书写日记() {
        ...
        await 易次元.创建选项("日记书写_返回按钮", {
            ...
            当点触结束时: async () => {
                ...
                易次元.播放音频("合上书本音效", { 资源标识: "$53383018" }); // resId: "$53383018"
                await 承诺.全部([
                    易次元.移除对象("日记书写背景图", { 效果: 易次元.消失效果.淡出, 时长: 1000 }),
                    易次元.移除对象("日记书写_交互按钮_图层", { 效果: 易次元.消失效果.淡出, 时长: 1000 }),
                ]);
        ...
        await 易次元.创建选项("日记书写_写日记按钮", {
            ...
            当点触结束时: async () => {
                ...
                await 易次元.创建文本样式("日记书写_静态段落文本样式", {
                    字体: "方正楷体",
                    字号: 30,
                    颜色: "#ffffff",
                    描边宽度: 0,
                ...
                await 易次元.创建文本样式("日记书写_动态段落文本样式", {
                    字体: "方正楷体",
                    字号: 30,
                    颜色: "#8faef2",
                    描边宽度: 0,
                ...
                if (日记配置.日记段落) {
                    易次元.移除对象("日记书写_交互按钮_图层", { 效果: 易次元.消失效果.淡出, 时长: 300 });

                    await 易次元.创建图片("日记书写_日记纸_背景图", {
                    ...
                    易次元.播放音频("翻书音效", { 资源标识: "$53260527" }); // resId: "$53260527"
                    await 易次元.显示对象("日记书写_日记纸_背景图", { 效果: 易次元.出现效果.淡入, 时长: 1000 });

                    const 日记纸图层 = "日记书写_日记纸_段落_图层";
                    await 易次元.创建图层(日记纸图层, {
                        层级索引: 100,
                        位置: { 横: 640, 纵: 360 },
                        锚点: { 横: 50, 纵: 50 },
                        裁剪模式: 是,
                    ...
                    for (const 段落 of 日记配置.日记段落) {
                        ...
                        if (段落.自动书写 === 否 && 索引 !== 0) {
                            ...
                            await new 承诺<无>((履行) => 易次元.创建选项("日记书写_日记段落_继续书写按钮", {
                                所属图层: 日记纸图层,
                                ...
                                位置: { 横: 900, 纵: 纵坐标 - 50 },
                                锚点: { 横: 50, 纵: 0 },
                                当点触结束时: () => (易次元.移除对象('日记书写_日记段落_继续书写按钮'), 履行()),
                            }));
                        }
                        ...
                        易次元.播放音频("写字音效", { 资源标识: "$53385360", 循环播放: 是 }); // resId: "$53385360"
                        await 易次元.创建文本(`日记书写_日记段落_${索引}`, {
                            所属图层: 日记纸图层,
                            ...
                            锚点: { 横: 50, 纵: 0 },
                            文本框大小: { 宽: 800, 高: 高度 },
                            水平对齐方式: 易次元.水平对齐方式.靠左,
                            是否可见: 否,
                        });

                        await 易次元.显示对象(`日记书写_日记段落_${索引}`, { 效果: 易次元.出现效果.淡入, 时长: 2000 });
                    ...
                    易次元.暂停音频("写字音效");
                    // rome-ignore format: 不需要换行
                    await new 承诺<无>((履行) => 易次元.创建选项("日记书写_日记段落_完成书写按钮", {
                        所属图层: 日记纸图层,
                        正常态资源标识: "$52601175", // resId: "$52601175"
                        点击态资源标识: "$52601175", // resId: "$52601175"
                        选项文字: "写完了       ",
                    ...
                    await 承诺.全部([
                        易次元.移除对象("日记书写背景图", { 效果: 易次元.消失效果.淡出, 时长: 1000 }),
                        易次元.移除对象("日记书写_交互按钮_图层", { 效果: 易次元.消失效果.淡出, 时长: 1000 }),
                        易次元.移除对象("日记书写_日记纸_背景图", { 效果: 易次元.消失效果.淡出, 时长: 1000 }),
                        易次元.移除对象(日记纸图层, { 效果: 易次元.消失效果.淡出, 时长: 1000 }),
                    ]);

                    await 易次元.打开对话框(易次元.预设对话框.旁白框, "我写了一份日记……并直接合上了日记本。");
                    await 易次元.关闭对话框();
                }
