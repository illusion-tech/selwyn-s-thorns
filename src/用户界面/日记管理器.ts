...
export class 日记管理器类 extends 事件目标 {
    ...
    async 书写日记(参数: { 背景底图资源标识: 资源标识 }) {
        ...
        const 日期 = this.#时间管理器.获取当前日期标准字符串();
    ...
    重置日记选项记录(参数: { 起始日期: Date; 是否含当日: 是否 }) {
        const 记录过滤函数 = (记录: 记录类) => {
            const 记录日期 = 记录.日期对象.getTime();
            const 参数日期 = 参数.起始日期.getTime();
            if (参数.是否含当日 === 是) return 记录日期 >= 参数日期;
            return 记录日期 > 参数日期;
        };

        const 待重置记录 = this.#选项记录器.获取记录().filter(记录过滤函数);

        for (const 记录 of 待重置记录) 记录.重置();
    }
}
