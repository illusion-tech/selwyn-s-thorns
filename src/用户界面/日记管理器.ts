...
class 日记页类 {
    ...
    static #贴纸位置: { [日期: 字符串]: { 横: 数值; 纵: 数值 } | 未定义 } = {
...
export class 日记管理器类 extends 事件目标 {
    ...
    #获取截至今日的日记页(入口日期 = "9999-12-31"): 日记页类 {
        ...
        const 污损页 = new 日记页类(this.#日记配置集.get(被污损) as 日记配置<unknown[]>);
        let 上一页 = 污损页;
        let 返回页 = 污损页;
    ...
    async #查看日记概要(入口页: 日记页类) {
        ...
        const 显示概要页 = async (日记页: 日记页类) => {
            ...
            await 接口.隐藏对象("日记概要_图层", { 效果: 常量.消失效果.淡出, 时长: 150 });
    ...
    async 打开日记本() {
        ...
        let 日记页: 日记页类 | 空白 = this.#获取截至今日的日记页();

        while (日记页 !== 空白) {
            if (日记页.日期 === 被污损) continue;
            if (!日记页.贴纸资源标识) throw alert(`日记本贴纸资源标识未配置: ${日记页.日期}`);
            if (!日记页.贴纸位置) throw alert(`日记本贴纸位置未配置: ${日记页.日期}`);

            const 入口页 = 日记页;

            await 接口.创建选项(`项链菜单_日记本_${日记页.日期}_选项`, {
                所属图层: "项链菜单_日记本_图层",
                正常态资源标识: 日记页.贴纸资源标识,
                点击态资源标识: 日记页.贴纸资源标识,
                位置: 日记页.贴纸位置,
