...
class 日记页类 {
    ...
    async 回溯当日(模式: '读档' | '重写') {
        ...
        if (剧情标识 === 未定义) return;
        const 记录集 = this.#选项记录器.获取记录().filter((记录) => 记录.日期 >= this.#日记配置.日期);
...
export class 日记管理器类 extends 事件目标 {
    ...
    async #回溯模式选择(所属图层: 字符串): Promise<'读档' | '重写'> {
        接口.创建图层("回溯模式选择_图层", {
            所属图层,
            裁剪模式: undefined
        });
    ...
    async #翻阅日记内容(日记页: 日记页类) {
        ...
        await 接口.创建选项("日记内容_返回按钮", {
            ...
            当点触结束时: () => {
                Promise.all([
                    接口.移除对象(日记内容图层, { 时长: 300 }),
                    接口.显示对象(日记概要图层, { 时长: 300 }),
                ]);
            },
        });

        await Promise.all([
            接口.隐藏对象(日记概要图层, { 时长: 300 }),
            接口.显示对象(日记内容图层, { 时长: 300 }),
        ]);
    }
    ...
    配置日记<类型 extends 未知[]>(...参数: { [属性 in keyof 类型]: 日记配置<[...类型[属性][]]> }) {
        for (const 日记配置 of 参数) {
            ...
            if (日记配置.是否可回溯 === 是 && 日记配置.回溯至 === 未定义) {
                throw new 错误(`日记配置 ${日期} 缺少回溯至配置！`);
            }

            if (日记配置.回溯至 !== 未定义 && 日记配置.回溯至.读档剧情标识 === 未定义) {
                throw new 错误(`日记配置 ${日期} 缺少读档剧情标识！`);
            }

            if (日记配置.回溯至 !== 未定义 && 日记配置.回溯至.重写剧情标识 === 未定义) {
                throw new 错误(`日记配置 ${日期} 缺少重写剧情标识！`);
            }
        ...
        await 接口.创建选项("日记书写_写日记按钮", {
            ...
            当点触结束时: async () => {
                ...
                if (日记配置.日记段落) {
                    ...
                    let 内容高度 = 0;
                    const 段落间距 = 35;
                    type 配置 = { 手动书写: 真假; 样式: 字符串; 文本: 字符串; 位置: 坐标; 大小: 大小 };
                    const 段落书写配置: 配置[] = [];
                    for (const [索引, 段落] of 日记配置.日记段落.entries()) {
                        ...
                        const 配置 = {
                            手动书写: 段落.自动书写 === 否 && 索引 !== 0,
                            样式: "判断条件" in 段落 ? "日记书写_动态段落文本样式" : "日记书写_静态段落文本样式",
                            文本: 内容,
