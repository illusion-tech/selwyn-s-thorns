    ...
    async 显示入口() {
        ...
        for (const 入口 of 地图.入口列表) {
            ...
            const 侦听器 = async () => {
                ...
                if (入口.去往.类型 === "地图") {
                    await Promise.all([
                        接口.移除对象(`地图_${地图.名称}_底图`, { 效果: 常量.消失效果.淡出, 时长: 1000 }),
                        接口.移除对象(`地图_${地图.名称}_地图层`, { 效果: 常量.消失效果.淡出, 时长: 1000 }),
                    ]);
            ...
            接口.添加事件侦听器(入口选项名称, 常量.事件类型.点触结束, 侦听器);
        }

        await 接口.显示对象(`地图_${地图.名称}_入口层`, {
            效果: 常量.出现效果.淡入,
            时长: 300,
        });
    }
