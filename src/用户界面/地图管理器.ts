...
export class 地图管理器类 extends 事件目标 {
    ...
    async 显示入口() {
        ...
        for (const 入口 of 地图.入口列表) {
            ...
            if (入口.提示) {
                ...
                await 易次元.创建文本(`地图_${地图.名称}_入口_${入口.名称}_提示`, {
                    所属图层: `地图_${地图.名称}_入口层`,
                    文本内容: 入口.提示.文本内容,
                    文本样式: "地图_提示文本样式",
                    文本框大小: 入口.提示.文本框大小,
                    位置: 提示文本框坐标,
                    垂直对齐方式: 易次元.垂直对齐方式.靠下,
                });
            }

            if (入口.禁用) {
                await 易次元.滤镜效果(入口选项名称, {
                    类型: 易次元.滤镜类型.灰度变化,
                    参数: 100,
                    时长: 100,
                });

                await 易次元.滤镜效果(`地图_${地图.名称}_场景入口_进入按钮`, {
                    类型: 易次元.滤镜类型.灰度变化,
                    参数: 100,
                    时长: 100,
                });
            ...
            const 侦听器 = async () => {
                ...
                if (入口.去往.类型 === "地图") {
                    ...
                    if (入口.描述界面) {
                        ...
                        await ac.show({ name: `地图_${地图.名称}_入口遮罩层` });
                    }
                    await 入口.执行?.();
                }
            ...
            易次元.添加事件侦听器(入口选项名称, 易次元.事件类型.点触结束, 侦听器);
        }

        await 易次元.显示对象(`地图_${地图.名称}_入口层`, {
            效果: 易次元.出现效果.淡入,
            时长: 300,
        });
    }
    ...
    async 显示地图(名称: 字符串, 过渡对象名称?: 字符串) {
        ...
        if (过渡对象名称) {
            await 易次元.对象过渡(过渡对象名称, `地图_${名称}_底图`, {
                通道图资源标识: "$51413", // resId: "$51413"
                时长: 1000,
            });
        } else {
            await 易次元.显示对象(`地图_${名称}_底图`, {
                效果: 易次元.出现效果.淡入,
                时长: 1000,
            });
        }

        await 易次元.创建图层(`地图_${名称}_地图层`, {
            层级索引: 100,
            是否可见: 否,
            裁剪模式: 是,
            裁剪区域: { 宽: 1280, 高: 720 },
        ...
        易次元.播放音频("地图出现音效", { 资源标识: "$51413" });
        await 易次元.显示对象(`地图_${名称}_地图层`, {
            效果: 易次元.出现效果.淡入,
            时长: 1000,
        });
        await this.显示入口();
